<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:1a0f00,50:4d2600,100:0d0700&height=220&section=header&text=DIYA%20R%20GOWDA&fontSize=60&fontColor=FF8C00&fontAlignY=42&animation=twinkling&desc=Building%20intelligent%20systems%20%7C%20AI%20%2F%20Multi-Agent%20%2F%20Full-Stack&descAlignY=62&descSize=17&descColor=FFB347" width="100%"/>

<br>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&duration=2500&pause=900&color=FF8C00&center=true&vCenter=true&width=800&height=45&lines=%3E+initializing+diya.dev...;%3E+loading+multi-agent+systems...;%3E+compiling+full-stack+applications...;%3E+status%3A+shipping+%F0%9F%9A%80" alt="Typing SVG" />

<br><br>

<img src="https://img.shields.io/badge/STATUS-ONLINE-FF8C00?style=for-the-badge&labelColor=1a0f00" />
<img src="https://img.shields.io/badge/CSE-3RD%20YEAR-FFB347?style=for-the-badge&labelColor=1a0f00" />
<img src="https://img.shields.io/badge/UNIVERSITY-PES-FF6B00?style=for-the-badge&labelColor=1a0f00" />

</div>

<br>

## `01` &nbsp;// SYSTEM.PROFILE

```python
class Diya:
    def __init__(self):
        self.currently_pursuing = "3rd year B.Tech, Computer Science Engineering @ PES University"

        self.currently_learning = (
            "Primarily working with JavaScript and Python, building end-to-end "
            "applications that span frontend UI, backend logic, and database integration"
        )

        self.identity = "AI & Multi-Agent Systems Enthusiast · Design-Conscious Developer"

        self.mission = "Ship and deploy open source projects to solve real world problems"

        self.currently_building = (
            "An autonomous repository comprehension agent + an AI-powered supplier risk platform"
        )

        self.exploring = ["LLMs", "Docker", "RAG", "TypeScript", "React", "Express", "MongoDB", "Cloud-Native Dev"]

        self.philosophy = "Building things that matter. Open to conversations that push that forward."

me = Diya()
```

<br>

## `02` &nbsp;// TECH.STACK

<div align="center">

**Languages**
<br>
<img src="https://skillicons.dev/icons?i=js,ts,py,java,cpp,c,go,rust,ruby,php,kotlin&theme=dark" />

**Frontend**
<br>
<img src="https://skillicons.dev/icons?i=html,css,react,nextjs,vite,tailwind,redux&theme=dark" />

**Backend & Frameworks**
<br>
<img src="https://skillicons.dev/icons?i=fastapi,flask,django,nodejs,express,dotnet&theme=dark" />

**AI / ML / Data**
<br>
<img src="https://skillicons.dev/icons?i=pytorch,tensorflow,sklearn&theme=dark" />
<img src="https://img.shields.io/badge/NumPy-FF8C00?style=flat-square&logo=numpy&logoColor=white" />
<img src="https://img.shields.io/badge/pandas-FF8C00?style=flat-square&logo=pandas&logoColor=white" />
<img src="https://img.shields.io/badge/LangChain-FF8C00?style=flat-square&logo=langchain&logoColor=white" />

**Databases**
<br>
<img src="https://skillicons.dev/icons?i=postgres,mysql,mongodb,sqlite,redis,dynamodb&theme=dark" />

**Cloud, DevOps & Tools**
<br>
<img src="https://skillicons.dev/icons?i=aws,azure,gcp,docker,kubernetes,githubactions,vercel,git,github,gitlab,vscode,linux&theme=dark" />

**Design & Collaboration**
<br>
<img src="https://skillicons.dev/icons?i=figma,notion&theme=dark" />
<img src="https://img.shields.io/badge/Slack-FF8C00?style=flat-square&logo=slack&logoColor=white" />

</div>

<br>

## `03` &nbsp;// ANALYTICS.DASHBOARD

<div align="center">

<img height="165" src="https://github-readme-stats-five-sigma-99.vercel.app/api?username=Diya-R-Gowda&show_icons=true&theme=tokyonight&border_color=FF8C00&bg_color=0d1117&title_color=FF8C00&icon_color=FFB347&count_private=true" />
<img height="165" src="https://github-readme-stats-five-sigma-99.vercel.app/api/top-langs/?username=Diya-R-Gowda&layout=compact&theme=tokyonight&border_color=FF8C00&bg_color=0d1117&title_color=FF8C00&langs_count=8" />

<br>

<img src="https://streak-stats.demolab.com?user=Diya-R-Gowda&theme=tokyonight&border=FF8C00&background=0D1117&ring=FFB347&fire=FF6B00&currStreakLabel=FF8C00" />

<br><br>

<img width="100%" src="https://github-readme-activity-graph.vercel.app/graph?username=Diya-R-Gowda&theme=react-dark&bg_color=0D1117&color=FF8C00&line=FFB347&point=FF6B00&hide_border=true&area=true" />

</div>

<br>

## `04` &nbsp;// CONNECT

<div align="center">

<a href="https://www.linkedin.com/in/diya-r-gowda-ab2394376/" target="_blank"><img src="https://img.shields.io/badge/LinkedIn-000000?style=for-the-badge&logo=linkedin&logoColor=FF8C00" /></a>
<a href="https://instagram.com/diyagowda_p.aw0627" target="_blank"><img src="https://img.shields.io/badge/Instagram-000000?style=for-the-badge&logo=instagram&logoColor=FF8C00" /></a>
<a href="mailto:diyagowda1427@gmail.com" target="_blank"><img src="https://img.shields.io/badge/Email-000000?style=for-the-badge&logo=gmail&logoColor=FF8C00" /></a>

<br><br>

<img src="https://komarev.com/ghpvc/?username=Diya-R-Gowda&label=PROFILE+VIEWS&color=1a0f00&style=for-the-badge&labelColor=FF8C00" />

</div>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d0700,50:4d2600,100:1a0f00&height=120&section=footer" width="100%"/>

<p align="center"><sub>⚡ compiled with ambition · <a href="https://github.com/Diya-R-Gowda">@Diya-R-Gowda</a></sub></p>
